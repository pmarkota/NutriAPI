...
namespace API.Controllers
{
    ...
    public class RecipesController : ControllerBase
    {
        ...
        )
        {
            ...
            {
                if (
                    !string.IsNullOrEmpty(filter.NameSearchTerm)
                    && filter.NameSearchTerm.Length < 2
                )
                {
        ...
        public async Task<ActionResult<RecipeReviewResponse>> CreateReview(
            Guid recipeId,
            [FromBody] RecipeReviewRequest review
        )
        {
            try
            {
                var createdReview = await _recipeService.AddRecipeReviewAsync(recipeId, review);
                return CreatedAtAction(
                    nameof(GetRecipeReviews),
        ...
        public async Task<ActionResult<IEnumerable<RecipeReviewResponse>>> GetRecipeReviews(
            Guid recipeId
        )
        {
            var reviews = await _recipeService.GetRecipeReviewsAsync(recipeId);
            return Ok(reviews);
        }
        ...
        )
        {
            try
            {
                var updated = await _recipeService.UpdateRecipeReviewAsync(reviewId, review);
                if (!updated)
                    return NotFound();
                return NoContent();
            }
            ...
            {
                return BadRequest($"Failed to update review: {ex.Message}");
            }
        }
        ...
        {
            var deleted = await _recipeService.DeleteRecipeReviewAsync(reviewId);
            if (!deleted)
                return NotFound();
            return NoContent();
        }
